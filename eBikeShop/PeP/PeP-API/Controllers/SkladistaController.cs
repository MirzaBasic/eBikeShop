...
namespace PeP_API.Controllers
{
    public class SkladistaController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Skladista/SearchByNameOrAddress/{name?}")]
        [ResponseType(typeof(Skladista))]
        public List<Skladista> GetSkladista(string name="")
        {
           return db.esp_Skladista_SelectByNameOrAddress(name).ToList(); ;
        ...
        public IHttpActionResult DeleteSkladista(int id)
        {
            ...
            db.Skladistas.Remove(skladista);
            db.SaveChanges();
